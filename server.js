...
app.post('/api/unified-auth/login', (req, res) => {
    ...
    if (role === 'staff') {
        ...
        req.session.deptId = deptId;
        req.session.serviceName = serviceName || null;
        const dept = requestStore.DEPARTMENTS.find(d => d.id === deptId);
        req.session.deptName = dept ? dept.name : deptId;
        req.session.loginAt = new Date().toISOString();
...
async function sendEmail({ to, subject, html }) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
            from: '노마 AI 복지 내비게이터 <onboarding@resend.dev>',
            to: recipients,
            subject,
            html,
        }),
    ...
    if (!res.ok) {
        ...
        throw new Error(`Resend API 오류 (${res.status}): ${errBody}`);
    }
...
const deptServiceMap = {
    ...
    '품질관리 종합 컨설팅사업': {
        ...
        deptId: 'private',
        phone: '055-230-8270',
        responsibility: '경상남도 내 민간 사회복지시설·제공기관 대상 집합컨설팅(노무·경영·재무회계·법률) 및 역량강화 교육 제공',
        basis: '경상남도사회서비스원 설립 목적 중 민간 사회복지시설 역량 강화. 운영 투명성 및 법적 준수 지원.',
        keyEligibility: ['경상남도 내 18개 시·군 사회서비스 제공기관', '사회복지법인 및 사회복지시설 종사자'],
    ...
    '1인가구 안전안심 라이프링 긴급구조 알림': {
        dept: '공공돌봄지원부',
        deptId: 'care',
        phone: '055-230-8200',
        responsibility: 'IoT 전등 센서·음성인식·블루투스 앱으로 고독사 위험 1인가구 응급상황을 조기 감지하고 신속 대응',
        basis: '경상남도 고독사 예방 정책. 1인 가구 안전 사각지대 해소를 위한 ICT 기반 돌봄.',
        keyEligibility: ['고독사 위험 1인 가구 (노인·장애인·중장년)', '경상남도 거주자', '총 100가구 선착순 선정'],
        keyQuestions: ['혼자 살고 계신가요?', '응급 상황 감지 장비가 필요하신가요?', '경상남도에 거주하시나요?'],
    },
    '맞춤형 통합지원': {
        dept: '민간지원협력부',
        deptId: 'private',
        phone: '055-230-8270',
        responsibility: '경남 내 민간 사회서비스 제공기관·종사자 대상 거점별 집합컨설팅·역량강화 교육으로 직무만족도 및 서비스 질 향상 지원',
        basis: '경상남도사회서비스원 민간기관 역량강화 사업. 종사자 처우개선 및 서비스 품질 향상.',
        keyEligibility: ['경상남도 내 18개 시·군 사회서비스 제공기관', '사회복지시설 및 사회서비스 제공기관 종사자'],
        keyQuestions: ['사회서비스 제공기관에서 근무하시나요?', '어떤 분야의 교육이나 컨설팅이 필요하신가요?'],
    },
    '사회서비스 정책연구 및 데이터 분석': {
        dept: '경상남도사회서비스원',
        deptId: 'research',
        phone: '055-230-8200',
        responsibility: '경상남도 사회서비스 수요·공급 실태 조사 및 정책 연구 보고서 발간',
        basis: '경상남도사회서비스원 설립 목적 중 정책연구 및 데이터 기반 의사결정 지원.',
        keyEligibility: ['지방자치단체, 사회복지기관, 복지 정책 관계자', '사회서비스 연구자, 관련 학계'],
        keyQuestions: ['어떤 분야의 사회서비스 데이터가 필요하신가요?', '연구 또는 정책 참고 목적인가요?'],
    },
};
...
■ 통합돌봄지원센터 (종합돌봄 연계)
  ...
  판별 키워드: 퇴원, 집에서, 계속 살고 싶어, 청소, 이동, 방문간호, 외롭
  수집 정보: 퇴원 여부/시점, 필요 서비스(이동·주거·간호·가사), 장기요양등급, 현재 이용 서비스, 독거 여부
  주의: 통합돌봄지원센터는 식사지원(밥·반찬 배달)을 직접 제공하지 않음. 식사지원이 필요한 경우 다른 기관 연계 안내
...
app.get('/api/smtp-check', requireAuth, async (req, res) => {
    ...
    const info = {
        ...
        RESEND_API_KEY: RESEND_API_KEY ? '설정됨' : '(미설정)',
        DEFAULT_RECIPIENTS,
    };
    if (RESEND_API_KEY) {
        try {
            await sendEmail({ to: DEFAULT_RECIPIENTS, subject: '[노마 AI] 이메일 연동 테스트', html: '<b>Resend API 이메일 발송 테스트 성공!</b>' });
            info.testSend = '성공';
        } catch (err) {
...
[최우선 규칙 - 응답 전 반드시 수행]
응답을 생성하기 전에 대화 히스토리를 검토하여 아래 정보가 이미 언급되었는지 확인하세요:
- 연령/나이 → 이미 답변함? → 다시 묻지 않음
- 거주지 → 이미 답변함? → 다시 묻지 않음
- 독거 여부/가구 구성 → 이미 답변함? → 다시 묻지 않음
- 이름 → 이미 답변함? → 다시 묻지 않음
- 전화번호 → 이미 답변함? → 다시 묻지 않음
- 현재 상황/어려운 점 → 이미 답변함? → 다시 묻지 않음
이미 파악된 항목은 절대 다시 질문하지 마세요. 아직 모르는 항목 중 필요한 것 1개만 질문하세요.
...
- [매우 중요] 서비스 추천 후, 반드시 "제가 바로 상담 신청을 도와드릴 수 있어요"라는 안내를 포함하세요. 단순히 전화번호만 안내하지 마세요.
- "신청"이 아닌 "상담 신청"이라고 표현하세요. 단, "비용", "무료" 등 금전 관련 단어를 먼저 언급하지 마세요. 사용자가 비용을 물어볼 때만 답변하세요.
- 예: "김영희님, 창원에 혼자 사시면서 퇴원 후 돌봄이 필요하시군요. 이런 서비스가 도움이 될 것 같아요. 원하시면 제가 바로 상담 신청을 도와드릴 수도 있어요!"
...
- 예: "직접 방문이나 전화도 가능하지만, 지금 저한테 말씀해 주시면 바로 상담을 접수해 드릴게요. 편하게 상담 신청하시겠어요?"

규칙:
- 한 번에 질문은 반드시 1개만 하세요. 여러 질문을 한꺼번에 나열하면 안 됩니다.
- 공감 → 질문 → (정보 충분 시) 추천 순서를 반드시 지키세요.
- 사용자가 답변할 때마다 공감 표현을 곁들이세요.
- [매우 중요 - 반복 금지] 대화 히스토리를 반드시 검토하고, 사용자가 이미 답변한 정보(연령, 거주지, 전화번호, 이름, 상황 등)는 절대 다시 묻지 마세요. 예: 사용자가 "75살이에요"라고 했으면 나이를 다시 묻지 않습니다. "창원이요"라고 했으면 거주지를 다시 묻지 않습니다.
- 전화번호 처리: 사용자가 전화번호를 말하면 숫자만 추출하여 010-XXXX-XXXX 형식으로 정리하세요. "공일공"은 010, "이"는 2, "삼"은 3으로 변환하세요. 한 번 확인 후 재차 묻지 마세요.
- 사용자가 "바로 추천해 주세요", "서비스 뭐 있어요?" 등 즉시 추천을 원하면 정보 수집을 생략하고 바로 추천해도 됩니다.
- 같은 내용을 반복하거나, 이전 응답과 비슷한 문장을 되풀이하지 마세요. 대화를 앞으로 진행하세요.

[응답 포맷 - 서비스 추천 시 반드시 아래 형식 사용]
- [매우 중요] 서비스를 추천할 때, 텍스트 본문에서 반드시 서비스 이름을 직접 언급하세요. "서비스가 있어요", "이 서비스들이" 같은 모호한 표현 대신, "응급안전안심서비스"와 "노인맞춤돌봄서비스"를 추천드려요 처럼 구체적으로 이름을 말하세요. <noma-card> 카드만으로는 사용자가 서비스명을 알 수 없습니다.
<noma-card>
{
  "serviceName": "사업명",
  "summary": "한 줄 요약",
...
[대화형 상담 신청 흐름]
사용자가 "상담 신청하고 싶어요", "담당자 연결해 주세요", "상담 받고 싶어요" 등의 의사를 밝히거나, 이름·전화번호를 자발적으로 알려주면 상담 신청 흐름으로 진입하세요.
"신청"이라는 단어 대신 항상 "상담 신청"이라고 표현하세요. 단, "비용", "무료", "돈" 등 금전 관련 단어를 먼저 언급하지 마세요.

[핵심 원칙] 이미 대화에서 파악한 정보(이름, 나이, 거주지, 상황, 전화번호)가 있으면 해당 단계를 건너뛰세요. 이미 아는 것을 다시 묻지 마세요.

1단계 - 신청 대상 서비스 결정:
- 직전 대화에서 추천한 서비스가 있으면 자동 선택하세요.
- 추천한 서비스가 없지만 사용자의 상황(예: 다리 아픔, 퇴원 후 돌봄)을 이미 파악했다면, 지식베이스에서 가장 적합한 서비스를 직접 선택하여 추천하고 바로 신청을 진행하세요. "어떤 서비스를 신청할까요?"라고 되묻지 마세요.
- 예: "말씀하신 상황으로 보면 '노인맞춤돌봄서비스'가 적합해요. 상담 신청을 도와드릴까요?"

2단계 - 아직 모르는 필수 정보만 수집 (serviceName, userName, userPhone):
- 대화에서 이미 파악된 정보는 스킵하고, 부족한 정보 1개만 질문하세요.
- 성함을 모르면: "상담 신청을 위해 성함을 알려주시겠어요?"
...
- [매우 중요] 대화 히스토리에서 사용자가 이미 말한 정보(이름, 나이, 전화번호, 거주지, 가구 구성)는 절대 다시 묻지 마세요. 해당 정보를 그대로 사용하세요. 예: 사용자가 "혼자 살아요"라고 했으면 "독거"로 기록.
- mainConcern은 대화 초반에 사용자가 설명한 어려움을 짧게 정리하세요 (예: "퇴원 후 돌봄 필요", "경제적 어려움").
- 응답 1개당 <noma-apply>는 최대 1개만 출력하세요.
- 전화번호 변환 규칙: "공"=0, "일"=1, "이"=2, "삼"=3, "사"=4, "오"=5, "육"=6, "칠"=7, "팔"=8, "구"=9. 예: "공일공 이삼사오 육칠팔구" → "010-2345-6789". 숫자와 하이픈만 남기고 정리하세요.
...
- 단, [Strict Grounding] 원칙은 그대로 지키세요. 검색 결과에 없는 서비스는 절대 추천하지 마세요.`;
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
            try {
                ...
                // 응답 완료 후 시스템 프롬프트 노출 여부 확인 → 감지 시 [BLOCKED] 전송
                if (checkResponseForLeaks(accumulatedResponse)) {
                    console.warn('[Security] 응답에서 시스템 프롬프트 노출 패턴 감지됨 — 차단');
                    if (!clientDisconnected) {
...
app.post('/api/service-request/connect', async (req, res) => {
    const { serviceName: rawServiceName, userName: rawUserName, userPhone: rawUserPhone, chatHistory } = req.body;

    // 입력값 sanitize
    const { text: serviceName } = sanitizeUserInput(String(rawServiceName || ''));
    const { text: userName } = sanitizeUserInput(String(rawUserName || ''));
    const { text: userPhone } = sanitizeUserInput(String(rawUserPhone || ''));
    ...
    const deptName = deptInfo?.dept || '미배정';
    const subject = sanitizeEmailHeader(`[상담 신청] ${serviceName} - ${deptName}`);
...
app.get('/api/staff/cases', (req, res) => {
    ...
    const cases = requestStore.getStaffCases(svcName);
...
app.patch('/api/case/:requestId/status', async (req, res) => {
    ...
    // 역행 방지: referred는 별도 상태이므로 제외하고 전진만 허용
    ...
    const updated = await requestStore.updateStatus(req.params.requestId, status);

    // 자동 연동: closed → 승인된 연계 executionStatus 일괄 completed
    if (status === 'closed') {
        const cnt = await requestStore.completeApprovedLinkages(req.params.requestId);
...
// 메모 추가 (author: '담당자')
app.post('/api/case/:requestId/notes', async (req, res) => {
    const { note } = req.body;
    if (!note || !note.trim()) return res.status(400).json({ error: '메모 내용을 입력하세요.' });
    const updated = await requestStore.addNote(req.params.requestId, note.trim(), '담당자');
...
// 연계 실행 상태 변경
app.patch('/api/case/:requestId/collaboration/:collabId', async (req, res) => {
    const { status } = req.body;
    ...
    const linkage = await requestStore.updateLinkage(req.params.requestId, req.params.collabId, { executionStatus: status });
    if (!linkage) return res.status(404).json({ error: '연계 요청을 찾을 수 없습니다.' });
    res.json(linkage);
});

// 연계 메모 추가
app.post('/api/case/:requestId/collaboration/:collabId/notes', async (req, res) => {
    const { text, author, dept } = req.body;
    if (!text || !text.trim()) return res.status(400).json({ error: '메모 내용을 입력하세요.' });
    const linkage = await requestStore.addLinkageNote(
...
// 연계 메모 추가
app.post('/api/case/:id/linkage/:lid/notes', async (req, res) => {
    const { text, author, dept } = req.body;
    if (!text || !text.trim()) return res.status(400).json({ error: '메모 내용을 입력하세요.' });
    const linkage = await requestStore.addLinkageNote(
...
// 관리자용 이상 건 알림
app.get('/api/admin/alert-items', (req, res) => {
    const items = requestStore.getAlertItems();
    res.json({ count: items.length, items });
});
...
// 서비스 계획 저장
app.post('/api/case/:id/service-plan', async (req, res) => {
    const { steps } = req.body;
    if (!steps || !Array.isArray(steps)) {
        return res.status(400).json({ error: '서비스 계획 단계가 필요합니다.' });
    }
    const plan = await requestStore.setServicePlan(req.params.id, { steps });
...
app.patch('/api/admin/requests/:id/status', async (req, res) => {
    ...
    const updated = await requestStore.updateStatus(req.params.id, status);

    // 자동 연동: closed → 승인된 연계 executionStatus 일괄 completed
    if (status === 'closed') {
        const cnt = await requestStore.completeApprovedLinkages(req.params.id);
...
// A-04: 메모 추가
app.post('/api/admin/requests/:id/notes', async (req, res) => {
    const { note } = req.body;
    if (!note || !note.trim()) return res.status(400).json({ error: '메모 내용을 입력하세요.' });
    const updated = await requestStore.addNote(req.params.id, note.trim());
    if (!updated) return res.status(404).json({ error: '요청을 찾을 수 없습니다.' });
    res.json(updated);
});
...
app.listen(PORT, () => {
    ...
    // Resend API 설정 확인
    if (RESEND_API_KEY) {
        console.log('[이메일] Resend API 설정 완료');
    } else {
