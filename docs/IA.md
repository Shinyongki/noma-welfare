...
│   │
│   ├── [Tab 4: 연계 — admin 역할] ───────────────────────────
│   │   ├── 연계 전체현황 (panel-collab) — 모니터링 전용, 승인 버튼 없음
│   │   │   └── 전체 연계 목록 (상태별 필터링)
│   │   └── 이상알림 (panel-alerts) — 장기 미처리·반복 반려 등 개입 필요 건
│   │
│   ├── [Tab 4: 연계 — dept 역할] ────────────────────────────
│   │   ├── 승인 대기 (panel-approvals)
│   │   │   └── 각 항목
│   │   │       ├── 사건 정보 (서비스명, 신청자, 접수일)
│   │   │       ├── 연계 유형 (자문/공동처리/이관/서비스연계) + 카테고리
│   │   │       ├── 요청 부서 → 대상 부서/서비스
│   │   │       ├── 사유
│   │   │       ├── 유형별 안내: 1단계(자문/서비스연계) — 승인 즉시 실행
│   │   │       │                2단계(공동처리/이관) — 승인 후 대상부서장 수락 필요
│   │   │       ├── 승인 타임라인
│   │   │       │   ├── 담당자 제출 (일시)
│   │   │       │   └── 부서장 처리 (일시 + 코멘트)
│   │   │       └── 액션 버튼 (승인/반려/수정요청 + 코멘트 입력)
│   │   └── 수신 요청 (panel-incoming) — 타부서가 보낸 공동처리/이관 요청
│   │       └── 각 항목: 수락 / 반려 + 코멘트
│   │
│   └── [Tab 4: 연계 — staff 역할] ──────────────────────────
│       └── (연계 탭 없음 — 케이스 목록 / 처리 완료 이력만 표시)
│
├── 📋 상담 처리 페이지 (/case/:requestId) ──→ case.html
│   │
│   ├── [로그인 게이트] ──────────────────────────────────────
│   │   ├── 역할 기반 로그인 (2단계 부서/사업 선택)
│   │   ├── 미로그인 시 기본 정보 열람 가능
│   │   └── 에러 메시지 표시
│   │
│   ├── [로딩 상태] (인증 후) ─────────────────────────────────
...
│       │   │       ├── 승인 상태 배지 (5종)
│       │   │       │   ├── 부서 검토 대기 (pending)
│       │   │       │   ├── 부서 승인 - 대상부서 수락 대기 (dept_approved)
│       │   │       │   ├── 최종 승인 (approved)
│       │   │       │   ├── 반려 (rejected)
...
│       │       ├── 사유 입력
│       │       ├── 요청 보내기 / 취소 버튼
│       │       └── 유형별 안내: 자문/서비스연계는 부서장 승인 즉시 실행,
│       │           공동처리/이관은 부서장 승인 후 대상부서장 수락 필요
│       │
│       │   [역할별 표시 조건]
│       │   ├── staff:    전체 기능 (연계 요청 생성 + 상태 진행 + 메모 등)
│       │   ├── dept:     열람 + 메모 + 승인 (연계 요청 생성 불가)
│       │   ├── admin:    전체 열람 + 상태 강제 변경
│       │   └── 미로그인:  기본 정보 열람만, 액션 잠금
│       │
│       ├── H. 배정 근거
│       │   ├── 담당 부서 정보 (부서명, 역할)
...
┌───────────┐  pending   ┌──────────────┐ dept_approved ┌──────────────┐ approved ┌──────────┐
│ 담당자     │ ────────→  │ 부서장       │ ───────────→  │ 대상부서장    │ ───────→ │ 실행     │
│ 연계 요청  │            │ 승인 대기     │               │ 수신 요청     │          │(이메일)  │
│ 생성      │            │              │               │ (수락/반려)   │          └──────────┘
└───────────┘            │ ┌── 승인 ───┐│               └──────┬───────┘
      ▲                  │ ├── 반려    ││                      │
      │   재제출          │ └── 수정요청 ┘│           rejected   │
...
관리자(admin)는 승인 흐름에 개입하지 않음
 → 연계 전체현황 모니터링 + 이상 건(장기 미처리, 반복 반려 등) 개입만
```
...
linkage = {
  ...
  reason:          "사유",
  submittedBy:     "제출자",
  approvalStatus:  "pending|dept_approved|approved|rejected|revision_requested",
...
### 5.3 admin.html 사이드바 네비게이션 (역할별)

| 역할 | 메뉴 | 탭 ID | 아이콘 |
|------|------|-------|--------|
| admin | 현황판 | panel-dashboard | monitoring |
| admin | 전체 상담요청 | panel-requests | list_alt |
| admin | 분석 | panel-analytics | analytics |
| admin | 연계 전체현황 | panel-collab | hub |
| admin | 이상알림 | panel-alerts | warning |
| dept | 소속부서 현황 | panel-dashboard | monitoring |
| dept | 승인 대기 | panel-approvals | approval |
| dept | 수신 요청 | panel-incoming | move_to_inbox |
| dept | 연계 이력 | panel-collab | hub |
| staff | 담당 케이스 | panel-cases | folder_open |
| staff | 처리 완료 | panel-completed | task_alt |
| 공통 | 메인으로 | / (링크) | home |

### 5.4 case.html 섹션 네비게이션

| 섹션 | 표시 조건 | 상호작용 | 역할별 접근 |
|------|----------|---------|------------|
| 대상자 정보 | 항상 | 읽기 전용 | 전체 (미로그인 포함) |
| AI 대화 요약 | conversationSummary 존재 시 | 읽기 전용 | 전체 (미로그인 포함) |
| 상태 스테퍼 | 항상 | "다음 단계" 버튼 (current 단계) | staff: 진행 가능 / dept·admin: 열람 / 미로그인: 열람 |
| 처리 메모 | 항상 | 메모 입력 + 추가 | staff·dept·admin: 읽기+쓰기 / 미로그인: 잠금 |
| 서비스 계획 | servicePlan 존재 시 | 읽기 전용 | staff·dept·admin / 미로그인: 잠금 |
| 연계 요청 | 항상 | 목록 조회 + 새 요청 생성 | staff: 전체 / dept: 열람+승인 (생성 불가) / admin: 열람+상태 강제 변경 / 미로그인: 잠금 |
| 배정 근거 | assignmentRationale 존재 시 | 읽기 전용 | staff·dept·admin / 미로그인: 잠금 |
| AI 상담 | 항상 | 역할 선택 + 질문 입력/빠른 질문 | staff·dept·admin / 미로그인: 잠금 |
| 연계 체인 | chain 존재 시 | 읽기 전용 | staff·dept·admin / 미로그인: 잠금 |
| 전화 버튼 | 항상 | tel: 링크 | 전체 (미로그인 포함) |
