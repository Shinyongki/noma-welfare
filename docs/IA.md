...
│
├── 🔔 토스트 알림 (전역, v012)
│   ├── 성공: 초록색 토스트 (상태 변경 완료, 저장 완료 등)
│   ├── 경고: 노란색 토스트
│   └── 오류: 빨간색 토스트
│   (confirm() 브라우저 다이얼로그 완전 대체)
│
├── 📧 서비스 연계 요청 페이지 (/referral/:requestId)
│   ├── 원래 상담 요청 정보 표시
│   ├── 연계 대상 서비스 선택 (카테고리별 드롭다운)
│   ├── 연계 사유 입력
│   └── 연계 이메일 발송 (Resend API) → 연쇄 연계 지원
│
├── 🔒 동의 모달 (첫 북마크 시)
│   ├── "서비스를 저장할까요?" 안내
│   ├── "네, 저장할게요" → 동의 후 저장
...
│   │   │       ※ 상태변경 완료 시 토스트 알림 표시 (v012, confirm() 제거)
...
│           └── 상태별 필터링 (요청/수락/완료/거절)
│
├── 🏢 부서 조정자 페이지 (/dept) ──→ dept.html
│   ├── [로그인 게이트] (DEPT_PASSWORD 인증)
│   └── [메인 콘텐츠] (인증 후)
│       ├── 부서별 연계/협업 승인 대기 목록
│       ├── 승인/반려/수정요청 처리
│       └── 배정 관리
...
```
[담당자 case.html]          [부서 조정자 admin/dept.html]       [관리자 admin.html]
                             연계 조정 탭                         연계 조정 탭
┌───────────┐              ┌──────────────┐               ┌──────────────┐
...
### 4.4 요청 데이터 (requestStore — JSON 파일, withLock 직렬화)
...
※ 모든 쓰기 작업은 withLock() 내에서 read-modify-write 직렬화 (v014)
※ 원자적 쓰기: tmp 파일 → rename (무결성 보장)
※ 자동 백업: .bak 파일 생성
```
...
### 4.6 분석 데이터 (analyticsStore — JSON 파일, withLock 직렬화)
...
※ analyticsStore도 withLock() 래핑 완료 (v014)
```
...
DEPARTMENTS = [
  ...
  { id: "dept-3", name: "사회서비스품질과",  type: "department" },
  ...
...
["아파"]
     │
     ▼ buildCumulativeQuery() ← v013: 이전 대화 맥락 누적
["아파"] + 이전 턴 키워드 누적
     │
...
["아파"] + 확장: ["질병","부상","돌봄","의료","긴급돌봄"]
     ...
     │  [v015 이후: pgvector cosine similarity 검색으로 대체]
┌──────────────────────────────┬────────┐
│ 긴급돌봄지원사업              │ 11.9점 │
│ 창원시/김해시 종합재가센터     │  8.4점 │
│ 경상남도통합돌봄지원센터       │  7.7점 │
...
└──────────────────────────────┴────────┘
     ...
     │
     ▼ withLock() → requestStore 저장 (원자적 쓰기 + .bak 백업)
     │   + AI 대화 요약/배정 근거 생성 (Gemini)
     │   + Resend API 이메일 발송 (담당 기관)
     │
     ▼ 담당자 이메일 수신 → case.html 링크 → 상담 처리
     │
     ▼ 연계 필요 시 → withLock() → linkage 생성 → 2단계 승인 → Resend 이메일
```
