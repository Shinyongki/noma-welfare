...
| 6 | | **buildCumulativeQuery()로 이전 대화 맥락 누적** → pgvector 3테이블 병렬 검색(welfare_kb 39건 + welfare_faq + welfare_docs 28건) |
...
| # | 시스템 |
|---|--------|
| 7a | 사회서비스원 소관 서비스 매칭 없음 감지 |
| 8a | AI가 읍면동 주민센터 또는 시군 복지과 안내 + 대표전화 제시 |

**A5. 통합돌봄 관련 질의 (v026)**
| # | 시스템 |
|---|--------|
| 6a | welfare_docs에서 정책 문서 매칭 (pgvectorDocSearch) |
| 7a | 통합돌봄 시스템 프롬프트 가이드 적용: 신청창구(읍면동)/긴급돌봄(사회서비스원)/모르면(대표전화) 3원칙 |
| 8a | AI가 통합돌봄 전체 vs 사회서비스원 역할 구분하여 안내 |
...
**A2. TTS 재생 중 사용자가 중단 (v032 강화)**
| # | 액터 | 시스템 |
|---|------|--------|
| 9a | 플로팅 마이크 버튼 탭 또는 탭 전환 | |
| 10a | | TTS 즉시 중단 (마이크 활성화·탭 전환 시 즉시 중단, v032) |
| 11a | | 음성 입력 모드로 전환 |
...
| 12 | | **requestStore.withLock()** 내에서 요청 저장 (원자적 쓰기 + 백업) |
| 13 | | **Resend HTTP API**로 담당 기관 이메일 발송 (신청 정보 + 대화 요약 + 배정 근거 + 연계 버튼 + case 처리 링크) |
...
| 10 | | **Resend HTTP API** 이메일 발송 (신청 정보 + AI 대화 요약 + 배정 근거 + 연계 버튼 + case 처리 링크) |
...
**A4. 통합돌봄 서비스 신청 시도 (v031)**
| # | 시스템 |
|---|--------|
| 2a | 신청 대상이 통합돌봄 서비스인 경우 감지 |
| 3a | AI가 `<noma-apply>` 태그 생성을 금지하고, 읍면동 주민센터 신청 안내로 대체 |
| 4a | "통합돌봄 서비스는 읍면동 주민센터에서 직접 신청하셔야 합니다" 안내 |
...
| # | 시스템 |
|---|--------|
| 1 | 음성 검색 완료 후 자동으로 TTS 실행. **정형 질문 5종은 사전 캐시로 즉시 재생 (v029)** |
| 2 | AI 요약 텍스트를 Edge TTS로 변환 (ko-KR-SunHiNeural). **첫 문장 조기 생성으로 응답 지연 최소화 (v029)** |
...
| 3 | | **buildCumulativeQuery()**: 이전 대화("아파") + 새 입력("72살", "혼자") 누적 → 확장 키워드 (독거노인, 1인가구, 고독사예방) |
...
| 7 | | 새 요청 ID 생성, 원래 요청에 연계 기록 추가 (withLock 직렬화) |
| 8 | | **Resend API** 연계 이메일 발송 (파란색 헤더, 연계 사유, 원래 신청 정보) |
...
|------|------|
| **액터** | 관리자, 담당자, 부서 조정자 |
| **목적** | 관리 페이지(admin.html, case.html, dept.html)에 접근하기 위해 인증한다 |
| **사전 조건** | ADMIN_PASSWORD / DEPT_PASSWORD 환경변수가 서버에 설정된 상태 |
| **사후 조건** | 세션 쿠키가 발급되어 보호된 API 접근 가능, 로그인 게이트가 닫히고 메인 콘텐츠 표시 |

### 기본 흐름
| # | 액터 | 시스템 |
|---|------|--------|
| 1 | /admin, /case/:id, /dept 접속 | |
| 2 | | GET /api/unified-auth/status 호출하여 기존 세션 확인 |
| 3 | | 세션 없음 → 로그인 게이트 표시 (비밀번호 입력 + 로그인 버튼) |
| 4 | 비밀번호 입력 | |
| 5 | 로그인 버튼 클릭 (또는 Enter) | |
| 6 | | POST /api/unified-auth/login 호출 |
| 7 | | 비밀번호 일치 → 세션 발급 (authenticated=true, 역할 설정, 8시간 유효) |
| 8 | | 로그인 게이트 숨김, 메인 콘텐츠(#main-content) 표시 |
| 9 | | 데이터 로드 시작 (admin: stats/requests/analytics, case: 사건 상세, dept: 승인 대기) |
...
| # | 시스템 |
|---|--------|
| 2a | GET /api/unified-auth/status → authenticated: true |
...
| 10 | | 해당 API 호출 → **토스트 알림** 표시 후 칸반 보드 갱신 (v012: confirm() 제거) |
...
| 12 | | 해당 API 호출 → **토스트 알림** 표시 후 테이블 갱신 |
...
| 10 | | **withLock()** 내에서 linkage 생성 (approvalStatus=pending, executionStatus=pending) |
...
| **사후 조건** | 1단계 완료 시 dept_approved → 관리자 검토 대기. 2단계 승인 시 Resend API 이메일 자동 발송. 반려/수정요청 시 담당자에게 재제출 요청 |

### 기본 흐름 (1단계: 부서 조정자 승인)
| # | 액터 | 시스템 |
|---|------|--------|
| 1 | admin.html 또는 dept.html → 연계 조정 탭 선택 | |
...
| 8 | | **withLock()** 내에서 approvalStatus: pending → dept_approved |
...
| 8 | | **withLock()** 내에서 approvalStatus: dept_approved → approved |
| 9 | | **Resend HTTP API**로 연계 대상 기관에 이메일 자동 발송 |
...
| 5 | | Gemini AI에 전송 (staffSystemPrompt + caseContext + kbSummary + pgvectorDocSearch 정책문서 + 대화) |
...
| 7 | | **withLock()** 내에서 requestStore에 servicePlan 저장 |
| 8 | | 저장 완료 표시 |
...
| 항목 | 내용 |
|------|------|
| **트리거** | 관리자/담당자/부서조정자 세션 생성 시 |
| **동작** | httpOnly + sameSite(lax) + secure(production) 쿠키로 8시간 유지 |
| **목적** | 세션 탈취 방지, CSRF 방어, 적절한 세션 수명 |

### NF-05. 데이터 안정성 (v014 강화)
| 항목 | 내용 |
|------|------|
| **트리거** | requestStore/analyticsStore 파일 쓰기 시 |
| **동작** | **withLock() 직렬화** + 원자적 쓰기(tmp→rename) + 자동 백업(.bak) + 손상 시 자동 복구 |
| **목적** | 동시 요청으로 인한 데이터 손상 방지 및 프로세스 중단 시 데이터 보호 |

### NF-06. 글로벌 에러 처리
| 항목 | 내용 |
|------|------|
| **트리거** | 라우트 핸들러에서 잡히지 않은 에러, unhandledRejection, uncaughtException |
| **동작** | 에러 로그 기록, 클라이언트에 500 응답, 프로세스 유지 |
| **목적** | 서버 갑작스러운 종료 방지, 에러 추적 가능 |

### NF-07. UI 알림 처리 (v012)
| 항목 | 내용 |
|------|------|
| **트리거** | 상태 변경, 일괄 처리, 저장 완료 등 사용자 액션 완료 시 |
| **동작** | **토스트 알림 UI** — confirm() 브라우저 다이얼로그 완전 제거 |
| **목적** | UX 일관성 확보, 비동기 처리 중단 없이 피드백 제공 |

### NF-08. TTS 최적화 (v029, v032)
| 항목 | 내용 |
|------|------|
| **트리거** | 음성 검색 결과 TTS 재생 시 |
| **동작** | **사전 캐시**(정형 질문 5종 워밍업, ~0ms) + **첫 문장 조기 생성**(일반 응답 1~2초) + **상담 신청 구간 TTS 비활성화**(이름·전화번호 수집 단계 스킵) + **마이크·탭 전환 시 즉시 중단** |
| **목적** | 음성 응답 체감 지연 최소화, 음성·화면 불일치 해소, 사용자 중단 의사 즉시 반영 |

### NF-09. 소관 외 배너 판정 강화 (v028)
| 항목 | 내용 |
|------|------|
| **트리거** | 도민 검색 결과 표시 시 |
| **동작** | pgvectorTopScore 기반 → **Gemini 응답 내용 기반 3중 조건**(hasServiceCard && hasOutOfScopeLanguage && outOfScopeFlag) 모두 충족 시만 소관 외 배너 표시. NON_WELFARE_THRESHOLD 0.45→**0.55** 상향 |
| **목적** | 소관 외 배너 오판(false positive) 감소 |

### NF-10. 멀티턴 대화 이름 반복 해소 (v031)
| 항목 | 내용 |
|------|------|
| **트리거** | 멀티턴 대화에서 AI가 이미 수집한 이름을 다시 질문 |
| **동작** | chatHistory를 배열 형식으로 Gemini에 전달하여 대화 맥락 정확 유지 |
| **목적** | 사용자 경험 개선 — 동일 정보 반복 질문 방지 |
