...
- **2026년 3월 시행된 돌봄통합지원법** 관련 서비스 안내 체계 미흡
...
- **소관 외 안내**: 읍면동 주민센터 전화번호 DB + 통합돌봄법 지식베이스 탑재 예정 (v016~v017)
...
| F-02 | 의미 기반 매칭 (RAG) | 유의어 사전 + 어미 제거 + 복합어 분해로 행정 용어 매핑. v015에서 pgvector 임베딩 검색으로 전환 예정 |
...
| F-06a | 소관 외 안내 | 사회서비스원 소관 외 서비스 → 읍면동 주민센터·시군 담당 안내 (v017 예정) |
...
| ID | 요구사항 | 상세 |
|----|---------|------|
| F-15 | 카테고리 탭 네비게이션 | 공공돌봄 / 민간지원 / 국공립시설 컬러별 탭. 지자체 탭 추가 예정 (v016) |
...
| **지자체/통합돌봄** | 0건 | 25건+ | 돌봄통합지원법 서비스, 읍면동 주민센터 공통 안내 (v016 예정) |
| **읍면동 전화번호** | 0건 | ~350건 | 경남 읍면동 행정복지센터 기관명·전화번호 (행정안전부 공공데이터 15061082 활용) |
...
| `PORT` | 서버 포트 | 5000 |
| `SUPABASE_URL` | Supabase 프로젝트 URL | (v015 이후 필수) |
| `SUPABASE_KEY` | Supabase anon key | (v015 이후 필수) |
...
- 지식베이스: CSV/JSON 21건 (v015 pgvector 전환, v016 확장으로 해소 예정)
- 인증 체계: 단일 비밀번호 방식 (운영 시 개인 계정 + 역할 기반 접근제어 필요)
- 요청 저장소: JSON 파일 기반, withLock 원자화 완료 (운영 시 DB 전환 필요)
- 읍면동 전화번호: 행정안전부 공공데이터(15061082) CSV 다운로드 후 경남 필터링 필요
...
| Phase 4 (진행중) | Vector DB 전환(Supabase pgvector v015), 지식베이스 확장(통합돌봄법+읍면동 v016), 소관 외 라우팅(v017) | 🔄 작업지시서 완료 |
