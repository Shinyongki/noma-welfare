...
| **기술 스택** | Node.js/Express (ESM), Gemini 2.0 Flash (RAG + 멀티턴), Edge TTS (사전 캐시 + 조기 생성), Resend HTTP API, TailwindCSS (로컬 빌드), Web Speech API, Chart.js, express-session |
...
- 사회서비스원 **소관 외 서비스**(읍면동·지자체 복지)로 안내받지 못하는 사각지대 존재
- **2026년 3월 시행된 돌봄통합지원법** 관련 서비스 안내 — v026에서 통합돌봄 KB 18건 + FAQ 25건 + 정책문서 28청크 탑재 완료
...
- **멀티턴 RAG**: `buildCumulativeQuery()`로 대화 맥락 누적, 후속 질문에서도 초기 맥락 유지
- **음성 입출력**: STT(음성→텍스트) + TTS(텍스트→음성, 사전 캐시 + 조기 생성 + 마이크·탭 전환 시 즉시 중단)로 눈/손이 불편한 사용자도 이용 가능
...
- **통합돌봄 안내**: 통합돌봄법 KB 18건 + FAQ 25건 + 정책문서 28청크 + 3테이블 병렬 벡터 검색 (v026 완료)
...
| F-02 | 의미 기반 매칭 (RAG) | pgvector 3테이블 병렬 검색(welfare_kb + welfare_faq + welfare_docs) + 폴백: 유의어 사전 + 어미 제거 + 스코어링 |
| F-02a | 멀티턴 RAG | `buildCumulativeQuery()`로 대화 누적 컨텍스트 기반 RAG 쿼리 빌드 (v013 완료). 후속 질문에서도 초기 맥락 유지 |
...
| F-06a | 소관 외 안내 | 사회서비스원 소관 외 서비스 → 읍면동 주민센터·시군 담당 안내. **NON_WELFARE_THRESHOLD 0.55 + Gemini 응답 기반 3중 조건 판정 (v028)** |
| F-06b | 통합돌봄 안내 | 통합돌봄(돌봄통합지원법) 질의 시 신청창구(읍면동)·긴급돌봄(사회서비스원)·대표전화 3원칙 안내 (v026) |
...
| F-08 | 음성 출력 (TTS) | Edge TTS (ko-KR-SunHiNeural), AI 응답 + 추천 서비스명 + 연락처 읽어주기. **사전 캐시(정형 질문 5종 워밍업) + 첫 문장 조기 생성(v029) + 상담 신청 구간 TTS 비활성화(v029보충) + 마이크·탭 전환 시 즉시 중단(v032)** |
...
| F-14 | 상담 신청 (카드 버튼) | 카드 하단 버튼 → 모달에서 이름·전화번호 입력 → Resend API 이메일 발송 |
| F-14a | 대화형 상담 신청 | AI 대화 중 이름·전화번호 수집 → `<noma-apply>` 태그로 자동 신청 → 인라인 확인 카드. **통합돌봄 서비스는 noma-apply 생성 금지 (v031)** |
| F-14b | 이메일 알림 | **Resend HTTP API**로 담당 기관에 상담 신청 이메일 발송 + AI 대화 요약 포함 |
...
| ID | 요구사항 | 상세 |
|----|---------|------|
| F-21 | 세션 기반 인증 | express-session으로 관리자/담당자/부서조정자 인증. 비밀번호 로그인 → 8시간 세션 유지 |
| F-22 | 로그인 게이트 | admin.html, case.html, dept.html에 로그인 UI 게이트. 인증 전 메인 콘텐츠 숨김 |
...
| ID | 요구사항 | 상세 |
|----|---------|------|
| F-25 | 현황판 (Dashboard) | KPI 카드 4종(총 접수, 처리 중, 연계 활성, 완료율) + 칸반 보드(5단계 컬럼) + 14일 접수 추이 차트. **confirm() 제거 → 토스트 UI (v012)** |
| F-26 | 상담 요청 관리 | 필터(상태)/검색(이름·전화·서비스)/정렬/페이지네이션 + 상세 모달(상태변경, 메모, 연계체인) |
| F-27 | 분석 탭 | 서비스별 인기도 막대 차트 + 카테고리 분포 도넛 차트 + KPI 추이 + 일별 이벤트 추이 |
| F-28 | 연계 조정 탭 | 2단계 승인 대기 목록(부서→관리자) + 승인/반려/수정요청 처리 + 승인 타임라인 + **Resend API 이메일** 자동 발송(최종 승인 시) |
...
| F-39 | 관리자 최종 승인 | dept_approved → approved/admin_rejected/admin_revision_requested. 승인 시 **Resend API 이메일** 자동 발송 |
...
| 항목 | 요구사항 |
|------|---------|
| **성능** | AI 응답 SSE 스트리밍 (체감 지연 최소화), TTS 2회 재시도 + **사전 캐시(정형 질문 5종) + 첫 문장 조기 생성 (v029)**, Gemini 429 시 3초 백오프 재시도 |
...
| **보안** | 세션 기반 인증(httpOnly, sameSite, secure), CORS 오리진 제한, helmet, rate-limit, JSON 본문 1MB 제한, 보호 API requireAuth |
| **안정성** | **withLock() 직렬화 (v014)** + 원자적 파일 쓰기(tmp→rename) + 백업 자동 생성(.bak) + 손상 자동 복구 |
| **이메일** | **Resend HTTP API** (Railway SMTP 포트 차단 대응). 발신: onboarding@resend.dev |

---

## 6. 지식베이스 구조

| 카테고리 | 현재 건수 | 목표 건수 | 예시 서비스 |
|---------|----------|----------|-----------|
| **공공돌봄** | 약 10건 | 10건+ | 긴급돌봄지원, 종합재가센터, 통합돌봄지원센터, 노인맞춤돌봄, 응급안전안심서비스 |
| **민간지원** | 약 4건 | 4건+ | 사회복지시설 대체인력, 장기요양기관 컨설팅, 활동지원사 교육 |
| **국공립시설** | 약 3건 | 3건+ | 경상남도청어린이집, 보조기기센터, 피해장애인쉼터 |
| **통합돌봄** | 18건 | 18건 | 의료요양 통합돌봄 제도개요, 방문진료, 퇴원환자연계, 방문재활, 병원동행, 임종케어, 긴급돌봄 등 (v026 완료) |
| **통합돌봄 FAQ** | 25건 | 25건 | 신청관련, 대상자, 서비스내용, 절차판정, 긴급상황 (v026 완료) |
| **정책 문서** | 28청크 | 28청크 | 통합돌봄 표준교안 (추진배경, 법률지침, 시범사업, 업무절차, 사례) (v026 완료) |
| **Supabase 합계** | welfare_kb 39건 + welfare_faq + welfare_docs 28건 | — | pgvector 3테이블 병렬 검색 |
...
[도민 브라우저 - code.html]
     ...
     │                      │
     │                ┌─────┴──────┐
     │                │ 이메일 발송 │──→ 담당 기관 이메일 (Resend API)
     │                │ (Resend)   │
     │                └─────┬──────┘
     │                      │
     │                ┌─────┴──────┐
     │                │requestStore│──→ withLock() 직렬화  ← v014
     │                │            │    + 원자적 쓰기(tmp→rename)
     │                └────────────┘
     │
...
[담당자 브라우저 - case.html]
     │
     ├── 로그인 ──→ POST /api/unified-auth/login ──→ 세션 발급
     │
     ...
     │
     └── AI 상담 ──→ POST /api/staff/chat ──→ Gemini AI (사건 컨텍스트 + KB + pgvectorDocSearch)
...
[관리자 브라우저 - admin.html]
     ...
     └── 연계 조정 ──→ GET /api/admin/pending-approvals
                  ──→ POST /api/admin/linkage/:lid/approve (최종 승인 + Resend 이메일)
...
| 지표 | 측정 방법 | 목표 |
|------|----------|------|
| RAG 매칭률 | 일상어 쿼리 대비 관련 서비스 매칭 비율 | 80% 이상 (pgvector 전환 후 90%+ 목표) |
...
| 소관외 해결률 | 읍면동/지자체 안내 후 재문의 없는 비율 | 70% 이상 (v017 이후 측정) |
...
- 이메일 발신 주소: onboarding@resend.dev (운영 시 커스텀 도메인 등록 필요)
- 지식베이스: pgvector 3테이블 운영 중 (welfare_kb 39건 + welfare_faq + welfare_docs 28건)
- 인증 체계: 단일 비밀번호 방식 (운영 시 개인 계정 + 역할 기반 접근제어 필요)
- 요청 저장소: JSON 파일 기반, withLock 원자화 완료 (운영 시 DB 전환 필요)
- 읍면동 전화번호: 행정안전부 공공데이터(15061082) CSV 다운로드 후 경남 필터링 필요 (미탑재)

### 향후 로드맵

| 단계 | 내용 | 상태 |
|------|------|------|
| Phase 2 (완료) | XSS 방어(DOMPurify), 보안 헤더(helmet), Rate Limiting, TailwindCSS 로컬 빌드, Resend API 전환 | ✅ 완료 |
| Phase 3 (완료) | read-modify-write 원자화(withLock v014), RAG 멀티턴(buildCumulativeQuery v013), confirm()→토스트(v012) | ✅ 완료 |
| Phase 4 (완료) | Vector DB 전환(v015), 지식베이스 확장(v016), 소관외 라우팅(v017), FAQ 구조화(v018), 통합돌봄 KB+문서DB(v026) | ✅ 완료 |
| Phase 4.5 (완료) | 소관외 배너 3중 조건(v028), TTS 사전캐시+조기생성(v029), 상담신청 구간 TTS 비활성화(v029보충), 이름 반복 해소+통합돌봄 noma-apply 금지(v031), TTS 중단 강화(v032) | ✅ 완료 |
| Phase 5 | 개인 계정 + RBAC(역할 기반 접근제어), SMS 발송 연동 | 📋 계획 |
| Phase 6 | 다국어 지원 (영어, 중국어, 베트남어) | 📋 계획 |
