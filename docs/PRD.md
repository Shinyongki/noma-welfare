...
| **기술 스택** | Node.js/Express (ESM), Gemini 2.0 Flash (RAG + 멀티턴), Edge TTS, Resend HTTP API, TailwindCSS (로컬 빌드), Web Speech API, Chart.js, express-session |
...
| F-06a | 소관 외 안내 | 사회서비스원 소관 외 서비스 → 읍면동 주민센터·시군 담당 안내 |
...
| **Supabase 합계** | welfare_kb 47건 + welfare_faq + welfare_docs 28건 | — | pgvector 3테이블 병렬 검색 |
...
- 지식베이스: pgvector 3테이블 운영 중 (welfare_kb 47건 + welfare_faq + welfare_docs 28건)
